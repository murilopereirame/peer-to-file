...
//   node src/server/cli.ts add-token <username> [name]   (token printed once)
...
function usage (): never {
  ...
  cli.ts add-token <username> [name]  create an API token (printed once)
...
try {
  switch (command) {
    ...
    case 'add-token': {
      if (!arg1) usage()
      const token = db.createApiToken(arg1, arg2 || 'cli')
    ...
    case 'list-tokens': {
      for (const t of db.listApiTokens(arg1)) {
        ...
        console.log(`${t.id}\t${t.name}\tuser ${t.user_id}\tlast used ${lastUsed}`)
      }
      break
    }
    case 'del-token': {
